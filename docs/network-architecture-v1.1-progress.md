...
| FA-5 | Semi-Real Closed-Loop Calibration v1 | In Progress | Added optional `train.system_calibration` path-feedback gate over external `path-feedback-summary/v1` JSON; system summary combines v5 `calibration_recommendation`, teacher gates, path-feedback gates, stress/mixed-stress diagnostics, and machine-readable exclusion reasons. No new network, action-space expansion, GCS backend, or Ackermann/skid-steer feasibility solver is introduced | Validate full semi-real gate with `bash scripts/run_path_feedback_validation.sh --scenario-set all --diagnostic-profile all --top-k 3 --output-root outputs/path_feedback_validation_next_stage` |
